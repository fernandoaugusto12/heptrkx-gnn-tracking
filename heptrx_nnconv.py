...
def train(epoch):
    ...
    for i,data in enumerate(train_loader):
        data = data.to(device)
        batch_target = data.y
        batch_weights_real = batch_target*10
...
def test():
    ...
    print('scor', sum_correct,
          ...
          'sfn', sum_falseneg,
          'stot', sum_total)
    return sum_correct / sum_total, sum_falsepos / sum_total, sum_falseneg / sum_total, sum_truepos/(sum_truepos+sum_falsepos)


for epoch in range(1, 30):
    epoch_loss = train(epoch)
    test_acc, test_fp, test_fn, test_pur = test()
    print('Epoch: {:02d}, Epoch Loss: {:.4f}, Test: {:.4f}, FalsePos: {:.4f}, FalseNeg: {:.4f} Purity: {:,.4f}'.format(epoch, epoch_loss, test_acc, test_fp, test_fn, test_pur))
